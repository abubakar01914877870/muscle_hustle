...
class BlogPost:
    ...
    def create(db, title: str, content: str, author_id: str, author_name: str, 
        ...
        data_to_save = temp_post.to_dict()
        # Restore ObjectIds for storage
        if 'author_id' in data_to_save and isinstance(data_to_save['author_id'], str):
            data_to_save['author_id'] = ObjectId(data_to_save['author_id'])
        # _id is typically None here, generated by insert_one
        
        result = db.blog_posts.insert_one(data_to_save)
        data_to_save['_id'] = result.inserted_id
        
        # Re-initialize with DB result to have everything set correctly
        return BlogPost(data_to_save)
    ...
    def save(self, db) -> bool:
        ...
        try:
            ...
            if '_id' in data_to_save and isinstance(data_to_save['_id'], str):
                data_to_save['_id'] = ObjectId(data_to_save['_id'])
            if 'author_id' in data_to_save and isinstance(data_to_save['author_id'], str):
                data_to_save['author_id'] = ObjectId(data_to_save['author_id'])
            ...
            if self._id:
                ...
                data_to_save['created_at'] = self.created_at
                result = db.blog_posts.insert_one(data_to_save)
